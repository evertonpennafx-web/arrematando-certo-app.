...
export default function RelatorioPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Fetch using both id and access_token (new schema 'access_token')
        ...
        if (error || !data) {
          ...
          setState({
            ...
            status: "invalido",
            reportHtml: "",
            errorMessage: "Relatório não encontrado ou token expirado.",
            invalid: true,
          });
          return;
        }
  ...
  const StatusBadge = ({ type, text }) => {
    ...
    return (
      <span className={`px-4 py-1.5 rounded-full text-sm font-bold border ${styles[type] || styles.neutral} inline-flex items-center gap-2`}>
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          <header className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div>
              {state.status === "finalizado" && <StatusBadge type="success" text="Análise Concluída" />}
              {state.status === "erro" && <StatusBadge type="error" text="Erro na Análise" />}
              {state.status === "processando" && <StatusBadge type="process" text="Em Análise..." />}
          ...
          <main className="bg-white rounded-2xl shadow-xl overflow-hidden min-h-[600px] border border-gray-100 relative">
            ...
            {(state.invalid || state.status === "erro") && (
               <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-white z-10">
                 ...
                 <a href="/" className="mt-8 px-6 py-3 bg-[#d4af37] text-black font-bold rounded-lg hover:bg-[#b8941f] transition-colors">
                   Voltar ao Início
                 </a>
               </div>
            )}
            ...
            {state.status === "finalizado" && state.reportHtml && (
              <div className="p-8 md:p-12 animate-in fade-in duration-500">
                <div className="mb-8 p-4 bg-[#d4af37]/10 border border-[#d4af37]/30 rounded-lg flex items-start gap-3">
                   <div className="mt-0.5"><CheckCircle2 className="w-5 h-5 text-[#d4af37]" /></div>
                ...
                <div className="mt-12 pt-8 border-t border-gray-200 text-center">
                  ...
                  <p className="text-gray-600 mb-6">Tenha acesso a uma consultoria completa com nossos especialistas.</p>
                  <a href="/?plan=express#pricing" className="inline-block px-8 py-4 bg-black text-white font-bold rounded-lg hover:bg-gray-800 transition-all shadow-lg hover:shadow-xl">
